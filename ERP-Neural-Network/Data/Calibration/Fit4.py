...
def SimulateLIF(time, weight, tau_syn, t_pre, V_thresh):
    dt_arr     = np.diff(time)
...
class Objective:
    ...
    def __call__(self, params):
        weight, tau_syn, t_pre, V_thresh = params
        if tau_syn <= 0 or weight <= 0 or V_thresh <= V_LEAK:
            return 1e10
        V_sim = SimulateLIF(self.time, weight, tau_syn, t_pre, V_thresh)
...
def DoFit():
    ...
    MEASURED_TAU_MEM = 0.14743556

    bounds = [
        (0.01, 8),
        (MEASURED_TAU_MEM, MEASURED_TAU_MEM),
        (t_bound_lo, t_bound_hi),
        (V_thresh - 0.05, V_thresh + 0.05),   # ±50 mV around detected value
    ]
    ...
    if run_fit:
        ...
        weight_fit, tau_syn_fit, t_pre_fit, V_thresh_fit = local_result.x
    else:
        weight_fit, tau_syn_fit, t_pre_fit, V_thresh_fit = 1.1641, 0.18298, -5.28 / 1000, 0.84747412
    ...
    V_sim = SimulateLIF(time, weight_fit, tau_syn_fit, t_pre_fit, V_thresh)
